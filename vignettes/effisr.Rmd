...
```{r}
      ggplot(filter(trend_eu, !is.na(current_ba)), aes(day, current_ba, group = country, color = country)) + 
      geom_step() +
...
scale_x_date(expand=c(0,25), date_labels = '%b', date_breaks = '2 months') +
  ...
  geom_text(data=IT_cum[IT_cum$date=='1976-12-30',],aes(label=year), 
